...
rlp::Result<Discv4Pong> Discv4Pong::parse(rlp::ByteView raw) {
    ...
    BOOST_OUTCOME_TRY(decoder.read(pong.ping_hash));
    rlp::ByteView hash_bv(pong.ping_hash.data(), pong.ping_hash.size());
    ...
    rlp::ByteView hexp_bv(expiration.data(), expiration.size());
    pong.expiration = 0;
    ...
    if (!decoder.is_finished()) 
    {
        ...
        // TODO Fix decoder.read function
        std::array<uint8_t, 6> ers_erq_array;
        BOOST_OUTCOME_TRY(decoder.read(ers_erq_array));
        pong.ers_erq |= (uint64_t)ers_erq_array[0] << 40;
        pong.ers_erq |= (uint64_t)ers_erq_array[1] << 32;
        pong.ers_erq |= ers_erq_array[2] << 24;
        pong.ers_erq |= ers_erq_array[3] << 16;
        pong.ers_erq |= ers_erq_array[4] << 8;
        pong.ers_erq |= ers_erq_array[5] << 0;
...
rlp::DecodingResult Discv4Pong::parse_endpoint(rlp::RlpDecoder& decoder, Discv4Pong::Endpoint& endpoint)
{
    ...
    endpoint.udp_port = port[0] << 8;
    endpoint.udp_port |= port[1];
    
    // Parse TCP port  
    BOOST_OUTCOME_TRY(decoder.read(port));
    endpoint.tcp_port = port[0] << 8;
    endpoint.tcp_port |= port[1];
